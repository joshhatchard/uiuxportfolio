...
const PixelBlast = ({
  ...
  useEffect(() => {
    ...
    const needsReinitKeys: Array<"antialias" | "liquid" | "noiseAmount"> = [
      "antialias",
      "liquid",
      "noiseAmount",
    ];
    ...
    if (mustReinit) {
      ...
      const onPointerDown: EventListener = (event) => {
        ...
        const { fx, fy } = mapToPixels(pointerEvent);
      ...
      const onPointerMove: EventListener = (event) => {
        ...
        const pointerEvent = event as PointerEvent;
        const { fx, fy, width, height } = mapToPixels(pointerEvent);
        touch.addTouch({ x: fx / width, y: fy / height });
      };

      const pointerEventTarget: EventTarget =
        interactionMode === "window" ? window : renderer.domElement;
